// Force rebuild Sat Dec  6 04:55:00 EST 2025
...
// Helper for query params builder
class SupabaseQueryBuilder {
  ...
  async execute() {
    ...
    try {
      ...
      if (!response.ok) {
        // Handle 406 (Not Acceptable) which happens when .single() returns 0 or >1 rows
...
export const supabase = {
  auth: {
    ...
    getSession: () => {
      // Check for hash params from OAuth redirect
      ...
      if (accessToken) {
        ...
        // Clear hash to prevent reprocessing
    ...
    getUser: async () => {
      // First ensure we have a valid session (checks hash)
    ...
    getCurrentUserWithProfile: async () => {
      ...
      try {
        ...
        const session = JSON.parse(sessionStr);
        const response = await fetch(`${SUPABASE_URL}/rest/v1/user_profiles?id=eq.${user.id}`, {
    ...
    onAuthStateChange: (callback) => {
      const checkAuth = async () => {
        try {
          // Initialize session from hash if present
          supabase.auth.getSession();
      ...
      upload: async (path, file) => {
        ...
        try {
          ...
          return { 
            data: { 
              path: data.Key || path,
              fullPath: `${SUPABASE_URL}/storage/v1/object/public/${bucket}/${path}`
            }, 
            error: null 
          };
        } catch (error) {
